...
from ..version import Arank_version as ult_ver
...
def Arank_cmd(
    pattern=None, manager=False, Arank_bot=Arank_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await Arank_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await Arank_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        Button.url("Bot", "t.me/Kanishka_String_Bot?start="),
                        Button.url(
                            "Repl",
                            "https://t.me/Kanishka_String_Bot",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = (
                    "**Arank Client Error:** `Forward this to` @Carding_Chronicle\n\n"
                )
                ftext += "**Py-Arank Version:** `" + str(pyver)
                ftext += "`\n**Arank Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START Arank CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Arank Client Error:** `Forward this to` @Carding_Chronicle\n\n",
                        )
                else:
