...
if run_as_module:
    ...
    from ..version import __version__ as __pyArank__
    from ..version import Arank_version

    file = f"Arank{sys.argv[6]}.log" if len(sys.argv) > 6 else "Arank.log"
    ...
    LOGS.info(f"py-Arank Version - {__pyArank__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Arank Version - {Arank_version} [{HOSTED_ON}]")
