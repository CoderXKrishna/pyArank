...
def startup_stuff():
    ...
    if CT:
        urlretrieve(CT, "resources/extras/Arank.jpg")
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://telegra.ph/Arank-06-18-2\nSo Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autobot():
    from .. import udB, Arank_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await Arank_bot.start()
    ...
    if who.username:
        ...
        username = "Arank_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await Arank_bot(UnblockRequest(bf))
    await Arank_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await Arank_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await Arank_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await Arank_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await Arank_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await Arank_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await Arank_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await Arank_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await Arank_bot.get_messages(bf, limit=1))[0].text
    await Arank_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "Arank_" + (str(who.id))[6:] + str(ran) + "_bot"
        await Arank_bot.send_message(bf, username)
        await asyncio.sleep(1)
        nowdone = (await Arank_bot.get_messages(bf, limit=1))[0].text
        if nowdone.startswith("Done!"):
            token = nowdone.split("`")[1]
            udB.set_key("BOT_TOKEN", token)
            await enable_inline(Arank_bot, username)
            LOGS.info(
...
async def autopilot():
    from .. import asst, udB, Arank_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await Arank_bot.get_entity(channel)
        except BaseException:
    ...
    assistant = True
    try:
        await Arank_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await Arank_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/0b9d7bd278272147f1f3c.jpg", "channelphoto.jpg"
        )
        ll = await Arank_bot.upload_file(photo)
        try:
            await Arank_bot(
...
async def customize():
    ...
    try:
        ...
        if not Arank_bot.me.username:
            sir = Arank_bot.me.first_name
        else:
            sir = f"@{Arank_bot.me.username}"
        file = random.choice(
            [
                "https://telegra.ph/file/785fc492053b5d1c904b3.jpg",
                "https://telegra.ph/file/0b9d7bd278272147f1f3c.jpg",
                "resources/extras/Arank_assistant.jpg",
            ]
        )
        ...
        await asyncio.sleep(1)
        await Arank_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await Arank_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await Arank_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await Arank_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await Arank_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await Arank_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await Arank_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await Arank_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await Arank_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await Arank_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await Arank_bot.send_message(
            "botfather",
            f"✨ Powerful Arank Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @Mr_Mrs_Krishna ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in Arank_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"Arank - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying Arank Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://telegra.ph/file/bd8944ef0af2e300e9c2f.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Arank has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(Arank_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @Mr_Mrs_Krishna\n➖➖➖➖➖➖➖➖➖➖"
        BTTS, PHOTO = None, None
        prev_spam = udB.get_key("LAST_UPDATE_LOG_SPAM")
        if prev_spam:
            try:
                await Arank_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await Arank_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await Arank_bot.send_message(chat_id, MSG)
    ...
    try:
        ...
        await Arank_bot(JoinChannelRequest("@Carding_Chronicle"))
    except BotMethodInvalidError:
        pass
    except ChannelsTooMuchError:
        LOGS.info("Join @Carding_Chronicle to know about new Updates...")
    except ChannelPrivateError:
        LOGS.critical(
            "You are Banned from @Carding_Chronicle for some reason. Contact any dev if you think there is some mistake..."
        )
        import sys
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, Arank_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else Arank_bot
...
async def enable_inline(Arank_bot, username):
    bf = "BotFather"
    await Arank_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await Arank_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await Arank_bot.send_message(bf, "Search")
    await Arank_bot.send_read_acknowledge(bf)
