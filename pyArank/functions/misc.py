...
from pyArank.exceptions import DependencyMissingError
...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=Arank_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="Arank All Cmds", content=[txt])
    await eor(event, f"All Arank Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def _insta_login():
    if "insta_creds" in Arank_bot._cache:
        return Arank_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            Arank_bot._cache.update({"insta_creds": cl})
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(Arank_bot.uid, timeout=60 * 2) as conv:
