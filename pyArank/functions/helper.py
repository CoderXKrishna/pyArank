...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @Carding_Chronicle"
            await eod(ok, f"✓ `Arank - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("Arank-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="Arank-heroku.log",
            thumb="resources/extras/Arank.jpg",
            caption="**Arank Heroku Logs.**",
        )

        os.remove("Arank-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Arank {Arank_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Arank {Arank_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
def make_html_telegraph(title, author, text):
    ...
    page = client.post(
        title=title,
        author=author,
        author_url="https://t.me/Mr_Mrs_Krishna",
        text=text,
    )
    return page["url"]
...
async def restart(ult=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        if len(sys.argv) == 1:
            os.execl(sys.executable, sys.executable, "-m", "pyArank")
        else:
            os.execl(
                sys.executable,
                sys.executable,
