...
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=ArankUSERBOT/Arank/{}/message={}"
    req_link = chatbot_base.format(
        Arank_bot.me.first_name or "Arank user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = Arank_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{Arank_bot.me.username}" if Arank_bot.me.username else None
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="Arankuser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "Arank")
    else:
        kwargs["code"] = code
